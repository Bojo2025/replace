...
**GitHub Pages folder:** this `docs/` directory (GitHub only allows `/(root)` or `/docs`).
**Live URL:** https://bojo2025.github.io/replace/
...
Open **`docs/`** as the Cursor workspace root when continuing web work.
...
replace/                          Parent folder on the Mid-2015 Mac
  ...
  docs/                     <-- THIS web project
```

When copying to the M5: copy the whole **`docs`** folder (or the whole `replace` parent if you want both apps).
...
1. Create a GitHub repo (e.g. `brsss-replacement` or `docs`).
2. Push **contents of `docs/`** as the repo root (so `index.html` is at root).
...
s,o=load_timetable('docs/data/latest-20-april.xlsx')
...
| Git remote | `docs` may have local `.git` without `origin` yet - add remote on M5 when pushing to GitHub |

---

## 11. Transfer checklist (Mid-2015 Mac -> M5)

Copy **`docs/`** entire folder (USB exFAT is fine).

**Ready-made zip (on the Mid-2015 Mac):**

`docs/replaceweb-for-M5.zip`

Contains: `index.html`, `css/`, `js/`, `data/latest-20-april.xlsx`, `PROJECT_HANDOFF.md`, `README.md`.  
Copy that zip to the pen drive, unzip on the M5, open the `docs` folder in Cursor.

| Include | Optional / skip |
|---------|-----------------|
| All of `docs/` (or the zip above) | Parent `.deps/` (desktop only) |
...
2. Prefer editing the **web** app under `docs/` unless the user asks about the desktop PyQt app.
